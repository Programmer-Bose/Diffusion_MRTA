...
class TemporalUNet1D(nn.Module):
    def __init__(self, channels=C, context_dim=None, base_ch=32, time_emb_dim=64):
        ...
        emb_dim = time_emb_dim  # combined via addition
        ...
        # Up-sample concatenates the upsampled features (64 channels) with the skip
        # connection from the first down block (32 channels), yielding 96 channels.
    ...
    def forward(self, x, t, context):
        ...
        c_emb = self.context_mlp(context)          # (B, emb_dim)
        emb = t_emb + c_emb
        ...
        u = torch.cat([u, h1], dim=1) if u.shape[-1] == h1.shape[-1] else u
        # ensure length matches h1 for concat (h1 has full H, u should too after upsample)
