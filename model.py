...
# ============================================================
def sinkhorn_column_normalize(logits, num_iters=20, temperature=0.5, dim=-2):
    """
    Runs Sinkhorn normalization over the designated robot dimension.
    Using dim=-2 automatically targets the row (robot) dimension for both
    unbatched (R, T) and batched (B, R, T) configurations.
    """
    log_alpha = logits / temperature
    for _ in range(num_iters):
        log_alpha = log_alpha - torch.logsumexp(log_alpha, dim=dim, keepdim=True)
...
class ThreeHeadedDenoiser(nn.Module):
    def __init__(self, robot_feat_dim, task_feat_dim,
        ...
        # Diffusion conditioning components
        self.time_embed = nn.Sequential(
            nn.Linear(1, d_model),
            nn.GELU(),
            nn.Linear(d_model, d_model)
        )
        self.xt_embed = nn.Embedding(max_R + 1, d_model)  # +1 handles unassigned task states safely
    ...
    def forward(self, robot_feats, task_feats, pref, x_t=None, t=None, sample_velocity=True):
        """
        robot_feats: (B,R,robot_feat_dim)
        task_feats:  (B,T,task_feat_dim)
        pref:        (B,pref_dim)  -- reference-point / preference vector
        x_t:         (B,T) optional integer tensor of noisy categorical allocations
        t:           (B,) or (B,1) optional float/int tensor containing current diffusion step
        sample_velocity: bool, true to sample from beta distribution, false for mean
        ...
        # Incorporate noisy state categorical assignment history if provided
        if x_t is not None:
            # Re-map unassigned index values (e.g., -1) cleanly to index R
            x_t_mapped = torch.where(x_t < 0, torch.tensor(R, device=x_t.device), x_t)
            xt_emb = self.xt_embed(x_t_mapped)  # (B,T,d)
            t_emb = t_emb + xt_emb
        ...
        # Incorporate scalar continuous/discrete diffusion step parameters if provided
        if t is not None:
            if t.dim() == 1:
                t = t.unsqueeze(-1)
            t_emb_vec = self.time_embed(t.float()).unsqueeze(1)  # (B,1,d)
            x = x + t_emb_vec  # Broadly inject step info across sequence context
...
def decode_schedule(assign_logits, rank_raw, velocity, temperature=0.5):
    ...
    P = sinkhorn_column_normalize(assign_logits, temperature=temperature, dim=-2)  # (R,T)
