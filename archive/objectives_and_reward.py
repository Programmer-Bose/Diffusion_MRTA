...
# Penalty for each task left unassigned, scaled by fraction of total
# tasks missed. Prevents an incomplete schedule from looking "better"
# just because fewer tasks means lower makespan/energy.
_UNASSIGNED_PENALTY_WEIGHT = 50.0
...
def compute_objectives(scenario, schedule, velocity):
    ...
    workload_variance = float(total_Wh_np.var())
    ...
    T = scenario["T"]
    assigned_tasks = (schedule > 0).any(axis=0)   # [T] bool, True if any robot does it
    num_unassigned = int((~assigned_tasks).sum())
    unassigned_frac = num_unassigned / T if T > 0 else 0.0
...
def compute_rewards(
    ...
    unassigned_penalty_weight=_UNASSIGNED_PENALTY_WEIGHT,
):
    """
    ...
    if non_terminal.any():
        ...
        if "unassigned_frac" in objective_batch:
            unassigned_frac = np.array(objective_batch["unassigned_frac"], dtype=np.float64)[idx]
            sub_reward = sub_reward - unassigned_penalty_weight * unassigned_frac
