...
CONTROL_POINT_SUFFIXES = ["_control_points", "_manual_control_points"]
CONTROL_POINT_PATTERN = re.compile(r"__?(?:manual_)?control_points(?:_.*)?\.json$")
...
def is_control_point_file(filename):
    return CONTROL_POINT_PATTERN.search(os.path.basename(filename)) is not None
...
def find_robot_files(project_dir=PROJECT_DIR):
    ...
    for f in map_files:
        ...
        if is_control_point_file(base):
            continue
        map_number_str, robot_id = parse_map_filename(base)

        cp_glob = os.path.join(project_dir, f"map_{map_number_str}_robot_{robot_id}*control_points*.json")
        cp_candidates = sorted(glob.glob(cp_glob))
        for cp_file in cp_candidates:
            trajectory_id = trajectory_id_from_filename(cp_file)
            entries.append({
...
def process_all_robots():
    ...
    for entry in sorted(entries, key=lambda e: (e["map_number"], int(e["robot_id"]), e["trajectory_id"])):
        ...
        obstacles_by_map.setdefault(map_number, map_data.get("obstacles", []))
...
def visualize_all_maps(combined_df, robot_meta, obstacles_by_map, out_dir=OUT_DIR):
    ...
    for map_number in sorted(obstacles_by_map.keys()):
        ...
        robot_ids = sorted(map_df["robot_id"].unique(), key=int)
        unique_trajectories = sorted(map_df["trajectory_id"].unique())
        colors = plt.cm.tab10(np.linspace(0, 1, max(len(unique_trajectories), 3)))

        for i, trajectory_id in enumerate(unique_trajectories):
            traj = map_df[map_df["trajectory_id"] == trajectory_id]
            robot_id = traj["robot_id"].iloc[0]
            meta_key = f"{map_number}_{robot_id}"
            meta = robot_meta[meta_key]
            ax.plot(traj["x"], traj["y"], color=colors[i % len(colors)], label=f"{trajectory_id}", linewidth=2)
            ax.scatter(*meta["start_world"], color=colors[i % len(colors)], marker="o", s=80, edgecolor="k", zorder=5)
            ax.scatter(*meta["goal_world"], color=colors[i % len(colors)], marker="*", s=150, edgecolor="k", zorder=5)
            ax.annotate(f"S{robot_id}", xy=meta["start_world"], xytext=(5, 5), textcoords="offset points", color=colors[i % len(colors)], fontsize=10)
            ax.annotate(f"G{robot_id}", xy=meta["goal_world"], xytext=(5, -14), textcoords="offset points", color=colors[i % len(colors)], fontsize=10)

        ax.text(0.02, 0.98, f"Map {map_number}", transform=ax.transAxes, fontsize=14, fontweight="bold", va="top")
        ax.set_xlim(-2, 14)
        ax.set_ylim(-2, 14)
        ax.set_aspect("equal")
        ax.set_title(f"Multi-Trajectory Paths (Map {map_number})")
        ...
        for i, trajectory_id in enumerate(unique_trajectories):
            traj = map_df[map_df["trajectory_id"] == trajectory_id]
            ax2.plot(traj["t"], traj["speed"], color=colors[i % len(colors)], label=f"{trajectory_id}")
...
if __name__ == "__main__":
    ...
    trajectory_keys = [f"{entry['map_number']}_{entry['robot_id']}_{entry['trajectory_id']}" for entry in entries]
    print(f"Found maps: {map_numbers}")
    print(f"Found trajectory entries: {sorted(trajectory_keys)}")
